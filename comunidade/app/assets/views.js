...
(function () {
  ...
  GVSI.views.enviar = {
    ...
    render: async function (view, params) {
      ...
      view.innerHTML =
        ...
        '<main class="lg:pl-[360px] pt-20 pb-28 px-container-margin"><div class="max-w-3xl mx-auto space-y-lg">' +
          '<section class="grid grid-cols-2 gap-md">' +
            '<button type="button" id="act-camera" class="bg-surface-container-high rounded-xl p-lg flex flex-col items-center justify-center gap-md shadow-sm active:scale-[0.98] transition-all"><div class="w-16 h-16 rounded-full bg-primary text-on-primary flex items-center justify-center shadow-lg"><span class="material-symbols-outlined text-4xl">photo_camera</span></div><div class="text-center"><h3 class="font-headline-sm text-headline-sm text-primary">Foto</h3><p class="text-body-sm text-on-surface-variant">Câmera ou galeria</p></div></button>' +
            '<button type="button" id="act-audio" class="bg-secondary-container rounded-xl p-lg flex flex-col items-center justify-center gap-md shadow-sm active:scale-95"><div class="w-16 h-16 rounded-full bg-secondary text-on-secondary flex items-center justify-center shadow-lg"><span class="material-symbols-outlined text-4xl">mic</span></div><div class="text-center"><h3 class="font-headline-sm text-headline-sm text-on-secondary-container">Áudio</h3><p class="text-body-sm text-on-secondary-container/80">Arquivo de áudio</p></div></button>' +
          '</section>' +
          '<input id="file-camera" type="file" accept="image/*" capture="environment" class="hidden"><input id="file-audio" type="file" accept="audio/*" class="hidden">' +
      ...
      function clearSel() { selectedFile = null; selectedKind = null; document.getElementById('preview').classList.add('hidden'); document.getElementById('preview-empty').classList.remove('hidden'); document.getElementById('btn-send').disabled = true; var a = document.getElementById('preview-audio'); a.classList.add('hidden'); a.src = ''; }
      function showSel(file, kind) { selectedFile = file; selectedKind = kind; document.getElementById('preview-empty').classList.add('hidden'); document.getElementById('preview').classList.remove('hidden'); document.getElementById('preview-name').textContent = file.name + ' · ' + humanSize(file.size); var thumb = document.getElementById('preview-thumb'), audio = document.getElementById('preview-audio'); var url = URL.createObjectURL(file); if (kind === 'image') { thumb.innerHTML = '<img src="' + url + '" class="w-full h-full object-cover" alt="">'; audio.classList.add('hidden'); } else { thumb.innerHTML = '<span class="material-symbols-outlined text-[32px]">graphic_eq</span>'; audio.src = url; audio.classList.remove('hidden'); } document.getElementById('btn-send').disabled = false; }
      ...
      document.getElementById('btn-send').addEventListener('click', async function () {
        ...
        try {
          var ext = (selectedFile.name.split('.').pop() || (selectedKind === 'image' ? 'jpg' : 'm4a')).toLowerCase();
  ...
  GVSI.views.suporte = (function () {
    ...
    return {
      ...
      render: async function (view) {
        ...
        var self = S;
        view.innerHTML =
          '<header class="fixed top-0 left-0 right-0 z-50 bg-surface shadow-[0px_4px_20px_rgba(0,0,0,0.05)] h-14 flex items-center justify-between px-container-margin"><a href="#/" class="flex items-center gap-sm text-primary" aria-label="Voltar"><span class="material-symbols-outlined">arrow_back</span><span class="font-headline-sm text-headline-sm font-bold">Suporte · Atendimento</span></a><div class="flex items-center gap-xs"><button type="button" data-theme-toggle class="w-10 h-10 rounded-full flex items-center justify-center text-on-surface-variant hover:bg-surface-container-high transition-colors" aria-label="Tema"><span class="material-symbols-outlined" data-theme-icon>dark_mode</span></button><button type="button" data-signout class="w-10 h-10 rounded-full flex items-center justify-center text-on-surface-variant hover:bg-error/10 hover:text-error transition-colors" aria-label="Sair"><span class="material-symbols-outlined">logout</span></button></div></header>' +
        ...
        document.getElementById('convo-back').addEventListener('click', function () { document.getElementById('convo-panel').classList.add('hidden'); document.getElementById('list-panel').classList.remove('hidden'); });
