  ...
  GVSI.views.suporte = (function () {
    ...
    return {
      ...
      render: async function (view) {
        ...
        S = { destroyed: false, channels: [], currentTicket: null, filter: 'todos', tagFilter: null, seen: Object.create(null), convoChannel: null, tags: [], contactTags: {} };
        var self = S;
        view.innerHTML =
          ...
          '<div class="pt-14 lg:pl-[var(--side-w)] h-[100dvh] flex">' +
            '<aside id="list-panel" class="w-full lg:w-[380px] lg:border-r border-outline-variant flex flex-col shrink-0"><div class="p-sm border-b border-outline-variant"><div class="relative"><span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-outline text-[20px] pointer-events-none">search</span><input id="sup-search" type="text" autocomplete="off" placeholder="Buscar por nome, telefone ou e-mail" class="w-full bg-surface-container-low border border-outline-variant rounded-xl py-2 pl-10 pr-3 text-body-sm text-on-surface focus:ring-2 focus:ring-primary/30 placeholder:text-on-surface-variant"></div><button type="button" id="sup-new" class="w-full h-10 mt-sm rounded-xl bg-primary text-on-primary font-label-md flex items-center justify-center gap-1 active:scale-[0.98] transition"><span class="material-symbols-outlined text-[20px]">edit_square</span>Nova conversa</button></div><div class="p-sm flex gap-1 border-b border-outline-variant"><button data-filter="todos" class="flex-1 py-2 rounded-lg text-label-md font-label-md bg-primary text-on-primary transition-colors">Todos</button><button data-filter="pendentes" class="flex-1 py-2 rounded-lg text-label-md font-label-md text-on-surface-variant hover:bg-surface-container-high transition-colors">Pendentes</button><button data-filter="resolvidos" class="flex-1 py-2 rounded-lg text-label-md font-label-md text-on-surface-variant hover:bg-surface-container-high transition-colors">Resolvidos</button></div><div id="tag-filter" class="hidden gap-1 px-sm pb-sm pt-1 overflow-x-auto custom-scrollbar border-b border-outline-variant items-center whitespace-nowrap"></div><div id="ticket-list" class="flex-1 overflow-y-auto custom-scrollbar"><p class="p-lg text-center text-on-surface-variant text-body-sm">Carregando…</p></div></aside>' +
        ...
        function openNewConversation() {
          ...
          async function start(uid) {
            ...
            if (tk.data && !self.destroyed) { self.filter = 'todos'; openTicket(tk.data); }
          }
